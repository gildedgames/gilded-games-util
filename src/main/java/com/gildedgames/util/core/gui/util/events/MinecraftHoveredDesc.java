...
public class MinecraftHoveredDesc extends GuiEvent<Gui>
{
	...
	{
		this.dim().mod().width(this.text.scaledWidth() + 5).height(this.text.scaledHeight() + 6).flush();
	...
	public void draw(Graphics2D graphics, InputProvider input)
	{
		...
		this.textElement.dim().mod().x(input.getMouseX() + 15).y(input.getMouseY() - 9.0F).flush();
