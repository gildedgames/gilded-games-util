		...
		if (spawnY > this.entry.getMinimumHeight() && spawnY < this.entry.getMaximumHeight() && this.entry.shouldAttempt(world, this.posX, spawnY, this.posZ))
		{
			...
			try
			{
				...
				if (!(entity instanceof EntityLiving) || ((EntityLiving) entity).getCanSpawnHere())
				{
					...
					final List<EntityPlayer> listOfPlayers = world.getEntitiesWithinAABB(EntityPlayer.class, new AxisAlignedBB(this.posX - playerRange, 0, this.posZ - playerRange, this.posX + playerRange, world.getActualHeight(), this.posZ + playerRange));
